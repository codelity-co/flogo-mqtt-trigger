...
func (t *Trigger) getHanlder(handler *clientHandler, parsed Topic) func(mqtt.Client, mqtt.Message) {
	return func(client mqtt.Client, msg mqtt.Message) {
		...
		receivedTimestamp := float64(time.Now().UTC().UnixNano())/float64(1000000)
		
		params := parsed.Match(ParseTopic(topic))

		t.logger.Debugf("Topic[%s] - Payload Recieved: %s", topic, payload)

		result, err := runHandler(handler.handler, payload, topic, receivedTimestamp, params)
		if err != nil {
...
func runHandler(handler trigger.Handler, payload, topic string, receivedTimestamp float64, params map[string]string) (map[string]interface{}, error) {
	out := &Output{
		Message:     payload,
